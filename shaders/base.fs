...
void main() {
    float intensity = dot(normalize(fragNormal), vec3(0.0, 1.0, 0.0));
    vec3 color = mix(vec3(0.0, 0.0, 1.0), vec3(1.0, 0.0, 0.0), (fragPosition.y + 1.0) / 2.0);
    //finalColor = vec4(color * intensity, 1.0);
    finalColor = vec4(abs(fragNormal), 1.0);
}*/

#define COLOR 0
#define iResolution vec2(800, 800)
#define iTime 0.0
